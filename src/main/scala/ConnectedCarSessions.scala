...
object ConnectedCarSessions {
  ...
  class ConnectedCarAssigner(watermarkMillis: Long) extends AssignerWithPunctuatedWatermarks[ConnectedCarEvent] {
    override def checkAndGetNextWatermark(lastElement: ConnectedCarEvent, extractedTimestamp: Long): Watermark = {
      return new Watermark(extractedTimestamp - watermarkMillis)
    }
  ...
  def main(args: Array[String]): Unit = {
    ...
    val carData = env.readTextFile("/Users/suresk/dev/flink-training/data/carInOrder.csv")

    carData
      .map(ConnectedCarEvent.fromString(_))
      .assignTimestampsAndWatermarks(new ConnectedCarAssigner(25000))
      .keyBy(_.carId)
